...
Class Database{
    ...
    function products_show($category_id, $colors = "`color`",$materials = "`material`",$styles = "`style`",$brands = "`brand`" ,$price0 = "`0`" ,$price1 = "`100000000`" , $page = 1 ,$perpage =36){
        ...
        $query = "SELECT * FROM `products` WHERE `category_id` = $category_id AND `color` REGEXP ($colors) AND `material` in ($materials) AND `brand` in ($brands) AND `style` in ($styles) AND `price` BETWEEN $price0 AND $price1 LIMIT $page,$perpage ORDER BY `products`.`id` DESC";
        mysqli_query($this->connection, "SET NAMES utf8");
        $result = mysqli_query($this->connection,$query);
        return($result);
    }
